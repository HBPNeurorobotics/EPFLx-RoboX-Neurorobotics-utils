...
class SOM_autograduation():
	...
	def __init__(self):
		...
		self.N = 1
		self.tau = 100.0
		self.limit = 90 # (s) only integer value
	...
	def graduate_all_functions(self):
		...
		if(os.path.exists('SOM_graduation.csv')): os.remove('SOM_graduation.csv')
	...
	def note(self, t, variF):
		self.varis[t] = variF
		self.vours[t] = self.standard[t][0]
		
		if  (self.varis[t] < self.standard[t][0]): 	self.notes[t] = 6
		elif(self.varis[t] < self.standard[t][1]): 	self.notes[t] = 5
		elif(self.varis[t] < self.standard[t][2]): 	self.notes[t] = 4
		elif(self.varis[t] < self.standard[t][3]): 	self.notes[t] = 3
		elif(self.varis[t] < self.standard[t][4]): 	self.notes[t] = 2
		elif(self.varis[t] < self.standard[t][5]): 	self.notes[t] = 1
		else:                    			self.notes[t] = 0
		
		print "Result: ", variF,"/", self.standard[t][0], "; Note: ", self.notes[t]
