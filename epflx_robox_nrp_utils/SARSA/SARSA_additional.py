...
class SARSA_additional():
	...
	def print_Qvalue(self,Q,goal):
		...
		df2 = df2.style.applymap(lambda border_negative: self.output.style.applymap(border_negative)).applymap(color_negative).apply(background_gradient, cmap='PuBu', m=df2.min().min(), M=df2.max().max(),low=0,high=0.2, goal=goal).set_properties(**{'width': '100px', 'border': '3px 1px black solid !important',  'color': 'black !important'});
		display.display(df2)
