...
class SARSA_additional():
	...
	def upload_positions(self):
		...
		states = pd.read_csv(self.csv_positions, delimiter=',',header=0).values
	...
	def print_Qvalue(self,Q,goal,actions,csv_file='lattice.csv'):
		...
		if(csv_file==self.csv_lattice): print("The table below holds the maximum (since we use the greedy policy) expected reward (Q-value) among possible actions at each state. You should observe that the expected reward is increasing as you move closer to the goal. Black blocks represent unaccessible areas, e.g., walls. As they are not considered as states, they are not assigned any reward. Note also that the expected goal reward should be less than the reward of some neighbouring states. Indeed, if you make one step away from the goal to next state then you will be one step away from goal as step from neighboring state to the goal put you into the goal thus the reward is bigger there.")
