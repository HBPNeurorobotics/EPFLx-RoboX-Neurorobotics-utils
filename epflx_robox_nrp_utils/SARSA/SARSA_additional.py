...
class SARSA_additional():
	...
	def print_Qvalue(self,Q,goal,actions):
		...
		df = df.style.applymap(border_negative).apply(background_gradient, cmap='PuBu', m=df.abs().min().min(), M=df.abs().max().max(),low=0,high=0.2, goal=gvalue).set_properties(**{'width': '100px', 'border': '3px 1px black solid !important', 'color': 'black !important'});
		if(self.testfile=='SOM_data_lattice.csv'): display.display(df)
