...
class SARSA_additional():
	...
	def print_Qvalue(self,Q,goal,actions):
		...
		df4 = df4.style.applymap(border_negative).applymap(color_negative).apply(background_gradient, cmap='PuBu', m=df4.min().min(), M=df4.max().max(),low=0,high=0.2, goal=gvalue2).set_properties(**{'width': '100px', 'border': '3px 1px black solid !important'});
		display.display(df4)
